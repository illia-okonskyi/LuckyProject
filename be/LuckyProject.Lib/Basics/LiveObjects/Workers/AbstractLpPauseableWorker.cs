...
{
    public abstract class AbstractLpPauseableWorker : AbstractLpWorkerBase, ILpPausableWorker
    {
        #region Internals
        ...
        #region ctor
        protected AbstractLpPauseableWorker(string name = null, ILogger logger = null)
            : base(name, logger)
        { }
        #endregion
        ...
        protected LpPausebleWorkerCancelSource GetCancelSource()
        {
            if (IsStopBaseRequested())
            {
                return pauseCts.IsCancellationRequested
                    ? LpPausebleWorkerCancelSource.Pause
                    : LpPausebleWorkerCancelSource.Stop;
            }

            return LpPausebleWorkerCancelSource.Other;
        }
        ...
        protected async override Task WorkerAsyncImpl(CancellationToken cancellationToken)
        {
            ...
            try
            {
                await PausableWorkerAsync(Progress, combinedCts.Token);
            ...
            catch (OperationCanceledException ex)
            {
                ...
                if (cancelSource == LpPausebleWorkerCancelSource.Stop)
                {
                    exitCode = ExitCodes.Stopped;
                    Logger?.LogDebug("Stopped");
                    return;
                }
                else if (cancelSource == LpPausebleWorkerCancelSource.Pause)
                {
        ...
        protected abstract Task PausableWorkerAsync(
